...
if(substr(lipid_data$Annotation lipid_data$Annotation 
#
```
...
lipid_data_m <- data.matrix(lipid_data[,16:75])



rownames(lipid_data_m) <- as.vector(as.character(lipid_data$Identifier))

lipid_data_m <- lipid_data_m[-grep("iSTD", rownames(lipid_data_m)),-grep("Biorec",colnames(lipid_data_m))]
...
sig_lipid <- data.frame(uni_lipid[which(uni_lipid$`Pr(>Chisq)_overall`<= 0.1),"analyte"])

sig_lipid_data_m <- lipid_data_m[which(row.names(lipid_data_m) %in% sig_lipid$analyte),]
