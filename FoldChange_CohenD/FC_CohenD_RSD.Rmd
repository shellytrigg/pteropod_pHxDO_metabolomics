...
for (i in 1:nrow(lipid_data)){
  if(substr(lipid_data$Annotation[i],1,3)=="unk"){
...
Calculate mean FC
```{r}
metab_group_means <- metab_data_STACKED %>% group_by(BinBase.name, comment) %>% summarise(mean=mean(quant, na.rm = TRUE))
...
Calculate RSD
```{r}
metab_group_sd <- metab_data_STACKED %>% group_by(BinBase.name, comment) %>% summarise(mean = mean(quant,na.rm = TRUE),sd=sd(quant,na.rm = TRUE))

metab_group_sd$RSD <- metab_group_sd$sd/metab_group_sd$mean *100


metab_group_RSDs <- spread(metab_group_sd[,c("BinBase.name", "comment","RSD")], "comment", "RSD")
...
Cohen_HLeffect_metab = STACKED_metab_HL %>% group_by(BinBase.name) %>%  
  do(metab_cohen_HLeffect = cohen.d(quant ~ comment,hedges.correction=TRUE, data = . ,na.rm = TRUE))

Cohen_HLeffect_metab_summ <- matrix(0, ncol = 4, nrow = length(Cohen_HLeffect_metab$metab_cohen_HLeffect), dimnames = list(Cohen_HLeffect_metab$BinBase.name, c("cohen_d", "magnitude", "confidence_int_inf", "confidence_int_sup")))
...
Cohen_LHeffect_metab = STACKED_metab_LH %>% group_by(BinBase.name) %>%  
  do(metab_cohen_LHeffect = cohen.d(quant ~ comment,hedges.correction=TRUE, data = . ,na.rm = TRUE))

Cohen_LHeffect_metab_summ <- matrix(0, ncol = 4, nrow = length(Cohen_LHeffect_metab$metab_cohen_LHeffect), dimnames = list(Cohen_LHeffect_metab$BinBase.name, c("cohen_d", "magnitude", "confidence_int_inf", "confidence_int_sup")))
...
Cohen_LLeffect_metab = STACKED_metab_LL %>% group_by(BinBase.name) %>%  
  do(metab_cohen_LLeffect = cohen.d(quant ~ comment,hedges.correction=TRUE, data = . ,na.rm = TRUE))

Cohen_LLeffect_metab_summ <- matrix(0, ncol = 4, nrow = length(Cohen_LLeffect_metab$metab_cohen_LLeffect), dimnames = list(Cohen_LLeffect_metab$BinBase.name, c("cohen_d", "magnitude", "confidence_int_inf", "confidence_int_sup")))
...
cohen_all_indv_effects_metabs$BinBase.name <- rownames(cohen_all_indv_effects_metabs)
###########################################
###############For Lipids##################
###########################################
...
```{r}
metab_FC_RSD <- merge(metab_group_means[,-c(2:5)], metab_group_RSDs[,-c(2:5)], by = "BinBase.name")

metab_FC_RSD_cohen <- merge(metab_FC_RSD, cohen_all_indv_effects_metabs, by = "BinBase.name")
...
```{r}
metab_FC_RSD_cohen <- merge(metab_data[,c("BinBase.name","InChI.Key")], metab_FC_RSD_cohen, by = "BinBase.name")

colnames(metab_FC_RSD_cohen)[1] <- "analyte"
...
lipid_FC_RSD_cohen <- merge(lipid_data[,c("Identifier","Annotation","InChI.Key")], lipid_FC_RSD_cohen, by = "Identifier")
