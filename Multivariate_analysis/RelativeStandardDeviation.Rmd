...
```{r}
ag_RSD_sig <- merge(stats_d_cut, ag_RSD, by = "Identifier")
...
```{r}
#plot distribution as boxplots
ggplot(ag_RSD_sig, aes(Treatment,RSD, fill = Treatment)) + geom_boxplot(outlier.shape = NA) + geom_jitter(aes(color = method_simple),width = 0.2, alpha = 0.7) + theme_bw() + scale_color_manual(values = c("black", "blue", "gold3"))
```

look for correlation between pvalue or VIP and RSD
```{r}

ggplot(ag_RSD_sig[which(ag_RSD_sig$DO_p.value < 0.05),], aes(DO_p.value, RSD, color = Treatment)) + geom_point() + ggtitle("RSD x DO pval for p < 0.05")

ggplot(ag_RSD_sig[which(ag_RSD_sig$pH_p.value < 0.05),], aes(pH_p.value, RSD, color = Treatment)) + geom_point()+ ggtitle("RSD x pH pval for p < 0.05")

ggplot(ag_RSD_sig[which(ag_RSD_sig$pH_p.value < 0.05),], aes(pH.DO_p.value, RSD, color = Treatment)) + geom_point()+ ggtitle("RSD x pH*DO pval for p < 0.05")

ggplot(ag_RSD_sig[which(ag_RSD_sig$VIP_PLSDA_comp1 >= 1.5),], aes(VIP_PLSDA_comp1, RSD, color = Treatment)) + geom_point()+ ggtitle("RSD x PLSDA VIP for VIP >= 1.5")
