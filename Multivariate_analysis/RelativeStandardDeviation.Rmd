...
```{r}
#plot distribution as boxplots
ggplot(ag_RSD_sig[which(ag_RSD_sig$method_simple!="not_sig"),], aes(Treatment,RSD, fill = Treatment)) + geom_boxplot(outlier.shape = NA, alpha = 0.5) + geom_jitter(aes(color = method_simple),width = 0.2, alpha = 0.4) + theme_bw() + scale_color_manual(values = c("black", "gold3", "blue")) + ggtitle("RSD x treatment for significant lipids")
```
